<template>
  <div class="">
    ...
    <div v-for="book in pendingRequests" :key="book.id" class="mb-3">
      <div class="bg-blue-200 flex items-center justify-between px-3 py-5 rounded-xl border-blue-400 border-2">
  ...
  <div class="flex flex-col">
    ...
    <button @click="selectTutor(book.tutor_id)" class="border-blue-600 border text-blue-600 hover:text-blue-400 hover:border-blue-400 text-xs py-2 rounded-md">View Profile</button>
  </div>

  <div class="flex flex-col">
    <p>You requested for</p>
    <div class="flex justify-center py-1 px-3 rounded-full text-xs font-semibold bg-blue-100 text-blue-800 border border-blue-300 mt-1">
                {{ book.subject }}
              </div>
  </div>
</div>
