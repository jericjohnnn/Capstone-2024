...
// Computed property to filter rejected tutors only
const filteredTutors = computed(() => {
  return tutors.value.filter(tutor => tutor.approval_status === 'Rejected');
});

// Computed property to paginate filtered tutors
const paginatedTutors = computed(() => {
  const start = (currentPage.value - 1) * itemsPerPage;
  return filteredTutors.value.slice(start, start + itemsPerPage);
});

// Total pages based on the number of rejected tutors
const totalPages = computed(() => {
  return Math.ceil(filteredTutors.value.length / itemsPerPage);
});
...
async function acceptTutor(tutorId) {
  try {
    ...
    if (tutorIndex !== -1) {
      ...
      tutors.value.push(tutor); // Move to the end of the list
    }
  } catch (error) {
...
async function rejectTutor(tutorId) {
  try {
    ...
    // Optionally handle tutor rejection (e.g., remove from list or update status)
    fetchTutors(); // Refetch tutors to ensure the list is updated
  } catch (error) {
