...
const tutors = ref([]);
const currentTab = ref('rejected'); // Default to 'accepted' for the accepted tab
...
// Computed property to filter tutors based on the selected tab
const filteredTutors = computed(() => {
  if (currentTab.value === 'pending') return tutors.value.filter(tutor => tutor.approval_status === 'Pending');
  if (currentTab.value === 'accepted') return tutors.value.filter(tutor => tutor.approval_status === 'Accepted');
   if (currentTab.value === 'rejected') return tutors.value.filter(tutor => tutor.approval_status === 'Rejected');
...
// Helper function to capitalize status for comparison
